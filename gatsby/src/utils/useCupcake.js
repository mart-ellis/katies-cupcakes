...
export default function useCupcake({ cupcakes, values }) {
  ...
  function addToOrder(orderedCupcake) {
    setOrder([...order, orderedCupcake]);
  }
  ...
  async function submitOrder(e) {
    ...
    // gather all the data
    const body = {
      order: attachNamesAndPrices(order, cupcakes),
      total: formatMoney(calculateOrderTotal(order, cupcakes)),
      name: values.name,
      email: values.email,
    ...
    if (res.status >= 400 && res.status < 600) {
      ...
      // it worked!
      setLoading(false);
      setMessage('Success! Come on down for your cupcake');
    }
  }
